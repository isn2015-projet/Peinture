...
void _set_window(int W, int H, char *titre, Uint32 flags)
{
	...
	// Destruction de l'ancienne fenetre/renderer 
	if(SDL_screen!=NULL) {
    ...
    printf( (soft_renderer) ? "Rendu software, pas d'acceleration graphique\n" : "Acceleration graphique activee\n" );
...
{	
	if (__initialiser_fenetre_is_already_called == 25) {
		...
		SDL_RenderPresent( SDL_renderer );		
		 ...
		 ecrire_texte("initialiser_fenetre() n'a pas ete appelee.");
		 ecrire_texte("");
		 ecrire_texte("		   Cliquer pour terminer.");
		 fprintf(stderr,"initialiser_fenetre() n'a pas ete appelee.\n");
...
void dessine_texte_gras_DimitriNicolas(char *a_ecrire, int taille, POINT p, COULEUR C)
{
    int i;
    SDL_Color couleur;
    SDL_Surface *surf = NULL;
    ...
    // Ecriture du texte dans la SDL_Surface "texte" en mode Blended (optimal)
    if (pol) {
        TTF_SetFontStyle(pol, TTF_STYLE_BOLD);
        surf = ( soft_renderer )
        ? TTF_RenderText_Solid(pol, a_ecrire, couleur)      // Renderer software = rendu solide
        : TTF_RenderText_Blended(pol, a_ecrire, couleur);   // Renderer hardware = rendu alpha
    }
...
void ecrire_texte(char *a_ecrire)
{
	...
	if (verdana_ok)
	{
		if (a_ecrire)
		{
			if (fin+strlen(a_ecrire) > 256) {fprintf(stderr,"Chaine de caracteres trop longue\n"); return;}
...
void dessine_image_rzs( IMAGE im, POINT P, double angle, double k, SYMETRIE sym) {
	...
	SDL_RenderCopyEx(SDL_renderer, im->texture, NULL, &dest, -angle, NULL, (SDL_RendererFlip) sym );
